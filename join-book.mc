/give @a written_book[written_book_content=
{
    pages:
    [
        '
        [
            "",
            {
                "text":" \\u0020 \\u0020 \\u0020 ",
                "color":"black"
            },
            {
                "text":"POKÉJONG SMP",
                "bold":true,
                "underlined":true,
                "color":"black"
            },
            {
                "text":"\\n\\nWelcome to our minecraft server! We have the following mods installed:\\n\\n",
                "color":"black"
            },
            {
                "text":"\\u25fc Lifesteal",
                "color":"dark_red"
            },
            {
                "text":"\\n",
                "color":"black"
            },
            {
                "text":"\\u25fc Pokémon",
                "color":"dark_blue"
            },
            {
                "text":"\\n",
                "color":"black"
            },
            {
                "text":"\\u25fc Mahjong",
                "color":"dark_green"
            },
            {
                "text":"\\n",
                "color":"black"
            },
            {
                "text":"\\u25fc Flavor Seeker",
                "color":"red"
            },
            {
                "text":"\\n",
                "color":"black"
            },
            {
                "text":"\\u25fc Proximity Chat",
                "color":"dark_purple"
            },
            {
                "text":"\\n\\n",
                "color":"black"
            },
            {
                "text":"we use custom made mods, be sure to read on!",
                "italic":true,
                "color":"gray"
            }
        ]',
        '
        [
            "",
            {
                "text":" \\u0020 \\u0020 \\u0020 \\u0020",
                "color":"dark_red"
            },
            {
                "text":"LIFESTEAL",
                "bold":true,
                "underlined":true,
                "color":"dark_red"
            },
            {
                "text":"\\n\\nWhen you die, you lose and ",
                "color":"black"
            },
            {
                "text":"drop",
                "hoverEvent":
                {
                    "action":"show_text",
                    "contents":"hearts can be picked up and eaten to gain MAXHP"
                }
            },
            {
                "text":" 1 \\u2665 MAXHP. "
            },
            {
                "text":"(You do not drop hearts at 3 MAXHP)",
                "color":"gray"
            },
            {
                "text":" You can also barter with your own hearts! To extract 1 \\u2665 MAXHP into your inventory, type this in chat:\\n\\n",
                "color":"black"
            },
            {
                "text":"/ls withdraw 1",
                "color":"dark_red",
                "clickEvent":
                {
                    "action":"suggest_command",
                    "value":"/ls withdraw"
                }
            }
        ]',
        '
        [
            "",
            {
                "text":" \\u0020 \\u0020 \\u0020 ",
                "bold":true
            },
            {
                "text":"POKÉMON",
                "bold":true,
                "underlined":true,
                "color":"dark_blue"
            },
            {
                "text":"\\n\\nEvery monster in the game has been replaced with Pokémon! If you beat another player in a battle, you will steal 1 \\u2665 MAXHP from them!\\n\\n[TAB] : View Menu\\n[\\u2191\\u2193] : Select Pokémon\\n[ R ] : Send Out Mon",
                "color":"black"
            }
        ]',
        '
        [
            "",
            {
                "text":" \\u0020 \\u0020 \\u0020 ",
                "bold":true
            },
            {
                "text":"MAHJONG",
                "bold":true,
                "underlined":true,
                "color":"dark_green"
            },
            {
                "text":"\\n\\nRiichi Mahjong. Yep.\\nLosers pay winners \\u2665 MAXHP(s) based on uma. Table colors maps uma and game type:\\n\\n",
                "color":"black"
            },
            {
                "text":"Green ",
                "color":"dark_green"
            },
            {
                "text":" (+1/+0/-0/-1)\\n \\u0020 \\u0020 \\u0020 1 hand\\n\\n",
                "color":"black"
            },
            {
                "text":"Blue",
                "color":"blue"
            },
            {
                "text":" \\u0020 (+2/+1/-1/-2)\\n \\u0020 \\u0020 \\u0020 1 round (East)",
                "color":"black"
            }
        ]',
        '
        [
            "",
            {
                "text":" \\u0020 \\u0020 ",
                "bold":true,
                "color":"red"
            },
            {
                "text":"FLAVOR SEEKER",
                "bold":true,
                "underlined":true,
                "color":"red"
            },
            {
                "text":"\\n\\nThere are over 400 unique food items to craft! Use this command to see the unique foods you have eaten:\\n\\n",
                "color":"black"
            },
            {
                "text":"/foodhistory",
                "color":"red",
                "clickEvent":
                {
                    "action":"suggest_command",
                    "value":"/foodhistory"
                }
            },
            {
                "text":"\\n\\nWhenever this list reaches milestones you receive free \\u2665 MAXHP!",
                "color":"black"
            }
        ]',
        '
        [
            "",
            {
                "text":" \\u0020 \\u0020 \\u0020"
            },
            {
                "text":"PROXIMITY CHAT",
                "bold":true,
                "underlined":true,
                "color":"dark_purple"
            },
            {
                "text":"\\n\\nWe have proximity voice chat in this server!\\n\\n[ V ] : chat settings\\n[ N ] : mute mic\\n[ P ] : player list\\n\\nEvery Sunday 8PM EST we have a server assembly meeting at (0,64,0)! Be there and receive 1 free \\u2665 MAXHP!",
                "color":"black"
            }

        ]',
        '
        {
            "text":"Feel free to share this minecraft server with any close friends. Please don\'t post it in public forums or discords. Word of mouth ensures the server culture is stable and server maintenance isn\'t stressful, thanks!\\n\\n \\u0020 \\u0020 \\u0020 \\u0020 \\u0020 \\u3030@shahrose",
            "color":"dark_gray"
        }'
    ],
    title:"Pokéjong Server Guide",
    author:"shahrose"
},
custom_name='
[
    "",
    {
        "text":"Pokéjong Server Guide",
        "italic":false
    }
]',
lore=
[
    '
    [
        "",
        {
            "text":"A book outlining all the mods we have installed on this server :)",
            "italic":false
        }
    ]'
],
enchantment_glint_override=false,
unbreakable=
{
}
]

